...
```
room/
├── models.py            # 数据模型：Room、PlayerSession、RoomStatus/Event 枚举、常量
...
├── snapshot_manager.py  # 快照管理：序列化游戏对象、Redis 存取、玩家视角裁剪
├── push_service.py      # 推送服务：broadcast / unicast、连接池管理
...
└── tests/
    ├── helpers.py       # 测试工厂：make_room() / make_session() / run_async()
...
- 推送：`build_player_view(snapshot, user_id)` 裁剪出单玩家视角后 unicast

**注意**：AFTER_DISCARD 阶段，快照中的 `current_player` 存的是刚打牌的一方，但前端期望的是"当前可以行动的一方"（即可以 ron/skip 的对手）。`build_player_view()` 里有特殊翻转处理：
...
`get_user_active_room(user_id)` 扫描 `self.sessions`，找到该玩家已加入的房间。  
`connect()` 入口处若返回非 None，关闭连接（`WS_CLOSE_ALREADY_IN_ROOM`）。
...
await room_manager.disconnect(room_name, user_id, connection_id)
await room_manager.handle_action(data, room_name, user_id)
```

### `handle_action()` 路由规则

按 `room.status` 分发，各状态下合法 action：
...
测试文件在 `server/room/tests/`，从项目根目录运行：

```bash
uv run pytest server/room/tests/ -v
```

`helpers.py` 提供：
- `make_room(room_name, status, player_ids)` — 快速构造 Room 对象
- `make_session(user_id, room_name, online)` — 快速构造 PlayerSession 对象
- `run_async(coro)` — 执行异步协程（内部用 `asyncio.run()`，避免事件循环警告）
